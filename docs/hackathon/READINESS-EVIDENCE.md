...
| Public packaging | fresh-history public repo | `Mbuso-Harvey/nexusos-semantic` @ `b0fd3b9` — single-commit release history, exactly the boundary-clean release tree, Apache-2.0 detectable; tag `nexus-alexa-submission-v1` on the actual public commit; logged-out cold-clone → full Firefox + Windows + Bedrock run reached 5/6 with the Bedrock badge, CONFIRM beat fired and granted, exact text written to Notepad, and the Windows verify mismatch named in FRICTION-LOG F-015 (planner expects `dirty`/`hasEditor`, live adapter provably returns `value`/`windowId`/`method`; `live-combined.ts` asserts value-only) |
